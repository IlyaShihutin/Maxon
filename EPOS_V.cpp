...
int MotorEposCheckStateController( int MotorNum)
{
    ...
    WORD NumID=(WORD)DvigMex[n].NumContr;
    BOOL pIsEnabled=true;
    if (DvigMex[n].Handle!=0)
        {
            if(!VCS_GetEnableState(DvigMex[n].Handle, NumID, &pIsEnabled, &Result))
                {
    ...
    if (DvigMex[n].Handle!=0)
        {
            ...
            if (pIsEnabled==1)
                {
                    VCS_SetDisableState(DvigMex[n].Handle,NumID,&Result);
                }
        }
    else
        {
    ...
    if (DvigMex[n].Handle!=0)
        {
            ...
            if(!pIsEnabled)
            {
                if(!VCS_SetEnableState(DvigMex[n].Handle,NumID,&Result))
                    {
                        MotorEposCheckResult(MotorNum, Result);
                        return (-1);
                    }
...
int MotorEposNumReadTekCoord(int MotorNum, double *TekPos)
{
    ...
    if(!VCS_GetPositionIs(DvigMex[n].Handle, NumID, &pPositionIs, &Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
...
int MotorEposNumReadZdnCoord(int MotorNum, double *ZdnPos)
{
    ...
    double Par1;
    if(!VCS_GetTargetPosition(DvigMex[n].Handle, NumID, &pTargetPosition, &Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
...
int MotorEposNumClearFaults(int MotorNum)
{
    ...
    if (MotorEposCheckStruct(MotorNum,&n)==-1) return(-1);
    WORD NumID=(WORD)DvigMex[n].NumContr;
    if(!VCS_ClearFault(DvigMex[n].Handle, NumID, &Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
...
int MotorEposNumBase(int MotorNum,__int8 m_uMode)
{
    ...
    MotorEposRaschMoveCoordPar(MotorNum, HPosition, &Par2,  'K',  0);

    if(!VCS_ActivateHomingMode(DvigMex[n].Handle, NumID,&Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
    if(!VCS_FindHome(DvigMex[n].Handle, NumID, m_uMode, &Result))
        {
            MotorEposCheckResult(MotorNum,Result);
            return(-1);
        }
    if(!VCS_WaitForHomingAttained(DvigMex[n].Handle, NumID, TimeOtr,&Result))
        {
            MotorEposCheckResult(MotorNum,Result);
            return(-1);
        }
    if(!VCS_GetHomingState(DvigMex[n].Handle, NumID, &pHomingAttained,&pHomingError, &Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
...
int MotorEposNumReadParBase(int MotorNum, double *SpeedSwitch, double *SpeedIndex, double *Uskor,double *HPosition, double *Offset)
{
    ...
    if(!VCS_GetHomingParameter(DvigMex[n].Handle,NumID,&pHomingAcceleration,
...
int MotorEposNumWriteParBase(int MotorNum, double SpeedSwitch, double SpeedIndex, double Uskor,double HPosition, double Offset)
{
    ...
    if(!VCS_SetHomingParameter(DvigMex[n].Handle, NumID, pHomingAcceleration,  pSpeedSwitch, pSpeedIndex,
...
int MotorEposNumWriteParMove(int MotorNum, double Speed,double Uskor, double Torm, double Rivok)
{
    ...
    if(!VCS_SetPositionProfile(DvigMex[n].Handle, NumID,ProfileVelocity,ProfileAcceleration,ProfileDeceleration, &Result))
        {
            MotorEposCheckResult(MotorNum,Result); return(-1);
        }
...
int MotorEposNumReadParMove(int MotorNum, double *Speed, double *Uskor, double *Torm, double *Rivok)
{
    ...
    if(!VCS_GetPositionProfile(DvigMex[n].Handle, NumID,&ProfileVelocity,&ProfileAcceleration,&ProfileDeceleration, &Result))
        {
            MotorEposCheckResult(MotorNum,Result); return(-1);
        }
...
int MotorEposNumRelMove(int MotorNum, double DistMove, int Key)
{
    ...
    {
        MotorEposDiagnController(TxtDgnContr15);
        return (-1);
    }
    ...
    if(!VCS_ActivateProfilePositionMode(DvigMex[n].Handle, NumID,&Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
    ...
    if(!VCS_MoveToPosition(DvigMex[n].Handle, NumID,TargetPosition,TRUE,m_oImmediately,&Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
...
int MotorEposNumAbsMove(int MotorNum, double DistMove, int Key)
{
    ...
    if(!VCS_MoveToPosition(DvigMex[n].Handle, NumID,DistMove,TRUE,m_oImmediately,&Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
...
int MotorEposNumStop(int MotorNum)
{
    ...
    if (MotorEposCheckStruct(MotorNum,&n)==-1) return(-1);
    WORD NumID=(WORD)DvigMex[n].NumContr;
    if(!VCS_HaltPositionMovement(DvigMex[n].Handle, NumID,&Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
...
int MotorEposNumWaitEndMove(int MotorNum)
{
    ...
    for (int i=0; i<KolCikl; i++)
        {
            if(!VCS_GetMovementState(DvigMex[n].Handle, NumID, &pTargetReached,&Result))
                {
                    MotorEposCheckResult(MotorNum, Result);
                    return (-1);
                }
...
int MotorEposNumReadState(int MotorNum, int *State)
{
    ...
    if(!VCS_GetObject(DvigMex[n].Handle, NumID, 0x6041, 0x00, &statusword_, 2,&pNbOfBytesRead, &Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
    VCS_GetOperationMode(DvigMex[n].Handle, NumID,  &pMode, &Result);
...
int MotorEposNumReadSensor(int MotorNum, int *Sens)
{
    ...
    if(!VCS_GetAllDigitalInputs(DvigMex[n].Handle,NumID,&pInputs,&Result))
        {
            MotorEposCheckResult(MotorNum, Result);
            return (-1);
        }
