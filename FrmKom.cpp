...
void __fastcall TFormFrmKom::Button19Click(TObject *Sender)//чтение мин и макс координаты
{
    MotorNum=MotorNumEdit->Value;
    double Max,Min;
    ...
    if (MotorEposNumReadState( MotorNum,&State)==0)
        {
            ...
            if(State & BIT16)//базирование
                {
                    CheckListBox1->Items->Strings[13] = "Ошибка поиска начальной точки";
                    CheckListBox1->Items->Strings[12] = "Достигнута начальная точка";
                }
            else //position mod
                {
                    CheckListBox1->Items->Strings[13] = "Ошибка следования";
                    CheckListBox1->Items->Strings[12] = "Подтверждение получения установочной точки";
                }
